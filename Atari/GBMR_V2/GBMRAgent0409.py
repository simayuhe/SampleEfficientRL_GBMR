...
from G_Dict0409 import Graph_dict
...
class GraphQAgent():
    ...
    def _train_g(self, states, actions, Q_targets):
        ...
        if not self.G.queryable(self.number_nn):
          return False
    ...
    def Reset(self, obs, train=True):
        ...
        return True
    def _predict_by_Graph(self,embedding):
        # Return action values for given embedding

        # calculate Q-values
        qs = []
        for a in range(self.n_actions):
            if self.G.queryable(self.number_nn):
              q = self.session.run(self.pred_q_G, feed_dict={
                self.state_embeddings: [embedding], self.action: [a]})[0]
            else:
              q = 0.0
    ...
    def Update(self, action, reward, obs, terminal=False):
        ...
        if self.training:
            ...
            if terminal:
                ...
                encoded_trj = self.G.add_by_trajectory(self.trajectory_embeddings,self.trajectory_actions, returns)
                if self.KF =="CrossNodes":
                ...
                print("Graph size",self.G.curr_capacity,"time using ",time_add_b-time_add_a)
    ...
    def ReconstructGraph(self,):
      if self.KF =="Degree":
        keypoints = self.G.GetKeyPointByDegree(10)#math.ceil(0.2*self.G.G.number_of_nodes()))
      if self.KF =="CrossNodes":
        keypoints,_ = self.keypoint.get_keypoint()
      ...
      for i in range(len(keypoints)):
          for j in range(len(keypoints)):
              if i==j:
                  ...
                  [nodes,actions,weights]=self.G.FindValuablePathbyLength(keypoints[i], keypoints[j])
                  if len(nodes)!=0:
                      self.G.update_by_indices(nodes, actions, weights)
