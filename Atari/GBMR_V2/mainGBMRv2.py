...
from GBMRAgent0409 import GraphQAgent 
...
def run_agent(args):
  ...
  with tf.Session(config=config) as sess:
    ...
    savename=args.save_path+'GBMRv2'+args.riqi+args.env
    ...
    for step in tqdm(range(training_iters), ncols=80):
        ...
        if terminal:
            ...
            C_time_b = time.time()
            agent.ReconstructGraph()
            C_time_a = time.time()
            print("reconstruction time using ",C_time_a-C_time_b)
            state = env.reset()
            agent.Reset(state)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--expert_memory_size', type=int, default= 10,
                       help='Number of test episodes per test')
    parser.add_argument('--KF', type=str, default='Degree',
                       help='Keypoints Finding algorithm')
